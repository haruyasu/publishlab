const translations = {
  en: {
    ...
    heroDescription:
      "A web application that allows you to upload and publish video content directly to your TikTok account through a secure, authorized connection using the official TikTok API.",
    featureAuthTitle: "Secure Authentication",
    featureAuthDesc:
      "Log in securely with your TikTok account using OAuth 2.0 without sharing your TikTok password.",
    featureUploadTitle: "Video Upload",
    featureUploadDesc:
      "Upload video files directly from your browser. Supports MP4, WebM, and MOV formats.",
    featurePublishTitle: "Direct Publishing",
    featurePublishDesc:
      "Publish videos directly to TikTok with captions, hashtags, and privacy settings.",
    howItWorksTitle: "How It Works",
    step1Title: "Connect Your TikTok Account",
    step1Desc:
      "Click the login button and authorize publishlab to access your TikTok account via the official TikTok OAuth flow.",
    step2Title: "Upload Your Video",
    step2Desc:
      "Select or drag-and-drop a video file from your device. Add a caption and hashtags for your post.",
    step3Title: "Publish to TikTok",
    step3Desc:
      "Configure your content and privacy settings and publish. Your video will be posted directly to your TikTok account.",
    getStartedTitle: "Get Started",
    loginDescription:
      "Connect your TikTok account to start uploading and publishing videos directly from publishlab.",
  ...
  ja: {
    ...
    heroSubtitle: "TikTok Video Publisher",
    heroDescription:
      "TikTokの公式APIを使用して、安全な認証接続を通じてTikTokアカウントに直接動画コンテンツをアップロード・公開できるWebアプリケーションです。",
    featureAuthTitle: "セキュア認証",
    featureAuthDesc:
      "OAuth 2.0を使用して、TikTokのパスワードを共有せずにTikTokアカウントに安全にログインできます。",
    featureUploadTitle: "動画アップロード",
    featureUploadDesc:
      "ブラウザから直接動画ファイルをアップロード。MP4、WebM、MOV形式に対応しています。",
    featurePublishTitle: "ダイレクト投稿",
    featurePublishDesc:
      "キャプション、ハッシュタグ、プライバシー設定付きでTikTokに直接動画を投稿できます。",
    howItWorksTitle: "使い方",
    step1Title: "TikTokアカウントを接続",
    step1Desc:
      "ログインボタンをクリックし、TikTokの公式OAuthフローでpublishlabがTikTokアカウントにアクセスすることを許可します。",
    step2Title: "動画をアップロード",
    step2Desc:
      "デバイスから動画ファイルを選択またはドラッグ＆ドロップします。投稿にキャプションとハッシュタグを追加できます。",
    step3Title: "TikTokに投稿",
    step3Desc:
      "コンテンツとプライバシー設定を構成して投稿します。動画はTikTokアカウントに直接投稿されます。",
    getStartedTitle: "はじめる",
    loginDescription:
      "TikTokアカウントを接続して、publishlabから直接動画のアップロードと投稿を始めましょう。",
...
const mockVideos = [
  {
    title: "Morning routine tips #lifestyle",
    date: "2026-02-18",
    views: 12400,
    likes: 580,
    comments: 42,
  ...
  {
    title: "Easy pasta recipe in 60 sec #cooking",
    date: "2026-02-15",
    views: 8700,
    likes: 320,
    comments: 28,
  ...
  {
    title: "Sunset timelapse Tokyo #travel",
    date: "2026-02-10",
    views: 23100,
    likes: 1200,
    comments: 95,
...
function renderVideoList() {
  ...
  if (mockVideos.length === 0) {
    container.innerHTML = '<p class="no-videos">' + t.noVideosYet + "</p>";
    return;
  }

  container.innerHTML = mockVideos
    .map(
      (v) =>
...
function showState(stateId) {
  ...
  // Show/hide tab bar based on logged-in state
  const tabBar = document.getElementById("tab-bar");
  if (tabBar) {
    const isLoggedIn = stateId === "state-form" || stateId === "state-videos";
    tabBar.style.display = isLoggedIn ? "flex" : "none";
  }
}
...
function initApp() {
  ...
  // Logout from videos tab
  const logoutBtnVideos = document.getElementById("btn-logout-videos");
  if (logoutBtnVideos) {
    logoutBtnVideos.addEventListener("click", () => {
      activeTab = "post";
      document.querySelectorAll(".tab-btn").forEach((btn) => {
        btn.classList.toggle("active", btn.dataset.tab === "post");
      });
      resetForm();
...
function resetForm() {
  ...
  if (uploadText)
    uploadText.textContent = translations[currentLang].videoFilePlaceholder;
  if (uploadIcon) uploadIcon.textContent = "\uD83C\uDFA5";
